/* global process */
/**
 * Trellis Context Injection Plugin (OpenCode 2)
 *
 * Injects context when the `subagent` tool is called with supported Trellis
 * agent types, and injects TRELLIS_CONTEXT_ID into `shell` tool commands.
 * Uses the OpenCode 2 `ctx.tool.hook("execute.before", ...)` API; the V1
 * `tool.execute.before` hook carried separate `(input, output)` objects, while
 * V2 hands one mutable `event` (`{ tool, sessionID, agent, messageID, id, input }`).
 *
 * V1 -> V2 rename map applied here:
 *   - tool `task` (args `subagent_type`/`prompt`)  -> `subagent` (args `agent`/`prompt`)
 *   - tool `bash`  (args `command`)                -> `shell`   (args `command`)
 */
...
// Supported subagent types (after stripping an optional "trellis-" prefix)
...
function getShellCommandKey(args) {
  if (!args || typeof args !== "object") return null
  if (typeof args.command === "string") return "command" // V2 `shell` tool
...
/**
 * OpenCode exports no session identity into Bash at all — it sets no session env
 * var in any process. The plugin hook does receive it (`event.sessionID`), so
 * inject it into `shell` tool commands before execution; that prefix is the only
 * channel by which an AI-run `task.py` sees the OpenCode session.
 */
function injectTrellisContextIntoShell(ctx, event, hostPlatform, env) {
...
// plugin context (an OpenCode server client + plugin extension API) and
// registers hooks on the domains that own them; registrations live for the
// plugin's lifetime. A plain object is accepted — `Plugin.define` from
// `@opencode/plugin` is only a TypeScript helper, and skipping it keeps these
// local plugins dependency-free (no node_modules needed under .opencode/).
...
export default {
  ...
  async setup(plugin) {
    ...
    await plugin.tool.hook("execute.before", async (event) => {
      try {
        if (env.TRELLIS_HOOKS === "0" || env.TRELLIS_DISABLE_HOOKS === "1") {
          return
        }
        debugLog("inject", "tool hook called, tool:", event?.tool)

        const toolName = String(event?.tool || "").toLowerCase()
        if (toolName === "shell") {
          if (injectTrellisContextIntoShell(ctx, event, hostPlatform, env)) {
        ...
        // V1's `task` tool used `subagent_type`; V2's `subagent` tool uses
        // `agent`. Strip a "trellis-" prefix from the v0.5.0-beta.5 agent
        // rename migration if present.
        const rawAgent = typeof args.agent === "string" ? args.agent : ""
        const subagentType = rawAgent.replace(/^trellis-/, "")
        ...
        if (!AGENTS_ALL.includes(subagentType)) {
          debugLog("inject", "Skipping - unsupported agent")
          return
        }
        ...
        // Mutate `args` (event.input) in place — the runtime holds a local
        // reference to the same input object.
        args.prompt = newPrompt

        debugLog("inject", "Injected context for", subagentType, "prompt length:", newPrompt.length)

      } catch (error) {
        debugLog("inject", "Error in tool hook:", error.message, error.stack)
      }
    })
  },
}
