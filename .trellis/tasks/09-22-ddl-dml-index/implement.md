...
- [ ] I5 单测新增：拆分（加列+加索引一表两条）、aspect 判定（index/primary/column/table 各 2 例）、维度过滤组合、复制=所见（过滤后导出文本逐行断言）；verbOf（七动词+OTHER 各 1 例）、动词过滤组合（ALTER 桶含列/主键/索引三类 ALTER，CREATE 桶不见 ALTER）。
