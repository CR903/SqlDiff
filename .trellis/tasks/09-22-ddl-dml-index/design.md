...
- `src-core/classify.ts`：加 `aspectOf(stmt)`：`/\bADD\s+(UNIQUE\s+|FULLTEXT\s+|SPATIAL\s+)?(INDEX|KEY)\b/i` 或 `DROP INDEX|KEY` → index；`PRIMARY KEY` → primary；`ADD/DROP/CHANGE/MODIFY COLUMN` → column；`CREATE TABLE`/`DROP TABLE` → table；其余 routine/data 由调用方定。
- `src-core/compare-filter.ts`：`postFilterResult` 加参 `{stmtKind?: 'ALL'|'DDL'|'DML', aspect?: 'ALL'|StmtAspect}`；data 行 stmtKind 恒 DML；`recountStats` 计 INDEX。
- `src-renderer`：维度切换 [全部/DDL/DML] + INDEX chip（与 CREATE/DROP/CHANGE、DML 三 Tab 正交，状态进 store，走同一 items 引用保证复制=所见）。
