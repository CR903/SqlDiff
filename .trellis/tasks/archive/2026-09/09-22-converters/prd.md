...
- R5 安全边界：导出的 JSON、toast、任务证据和下载文件不得包含密码、私钥、passphrase 或 Vault 密文；仅允许 DBeaver 的 `save-password: false` 控制字段，不得出现秘密值或秘密承载字段（如 `userPassword`、`keyValue`）。
...
- [x] AC1 节点选择弹窗可逐项勾选、全选/全不选，并能对至少 3 个样本节点生成导出。
- [x] AC2 生成文件是合法 JSON，包含 `folders`、`connections` 和必要的 MySQL `configuration` 字段；连接名与拓扑和 SqlDiff 节点一致。
- [x] AC3 SSH 节点生成 `ssh_tunnel` 拓扑；私钥路径不被编造；导出内容不含任何密码、私钥、passphrase 或密文。
- [x] AC4 下载文件真实落盘，文件名和内容符合 R2；UI 在导出前明确提示需要首连重输秘密。
- [x] AC5 单测、tsc、lint、test、build 和可信 CDP 回归通过；`mysqldiff/` 零改动。
- [x] AC6 DBeaver 真机导入验证：当前环境无 DBeaver，按用户批准的边界以结构化 JSON 校验作为本任务验收，并在报告中记录未完成的 IDE 真机验证；不得将结构校验表述为真机导入成功。
